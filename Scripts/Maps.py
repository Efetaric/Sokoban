...
def Maps_order(player, map):
    ...
    map_matrix[player.current_x][player.current_y].config(bg=map.Color_Player) #Places the player
...
def Standard_map(window, map):    
    ...
    for j in range(0,15):
        for i in range (0,15): 
            ...
            map_matrix[j][i]=Label(window, height=1, width=3, bg=map.Color_Space, relief=RAISED)
            map_matrix[j][i].grid(row=i, column=j, padx=1, pady=1)
            ##Makes the outter borders
            if (j==0 or j==14 or i==0 or i==14):
                map_matrix[j][i].config(bg=map.Color_Border)
